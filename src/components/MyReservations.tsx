          ...
          >
            ...
            <div className="relative">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span
                      className={`px-2 py-1 text-xs font-medium rounded-full ${
                        isToday ? 'bg-green-100 text-green-800' : isPast ? 'bg-gray-100 text-gray-800' : 'bg-blue-100 text-blue-800'
                      }`}
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-gray-600 flex items-center gap-2">
                      ...
                      <b>Turno:</b>{' '}
                      {reservation.shift === SumShift.Day ? 'Diurno' : reservation.shift === SumShift.Night ? 'Nocturno' : 'Día Completo'}
                    </p>
                    <p className="text-sm text-gray-600 flex items-center gap-2">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                      ...
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M2.25 21h19.5m-18-18v18m10-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21M3 3h12m-.75 4.5H21m-3.75 3.75h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z"
                        />
                      </svg>
                      <b>Salones:</b> {reservation.roomBig && reservation.roomSmall ? 'Ambos' : reservation.roomBig ? 'Grande' : 'Chico'}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                  {!isPast && <CVDeleteButton onClick={() => handleDelete(reservation.id)} className="px-2 py-1" />}
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t flex justify-between items-center">
                ...
                <div className="text-xs text-gray-500">Reservado el {new Date(reservation.reservedAt).toLocaleDateString()}</div>
              </div>
            </div>
          </div>
        )
