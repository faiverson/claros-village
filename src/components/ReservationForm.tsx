...
export default function ReservationForm({ existingReservations }: ReservationFormProps) {
  ...
  const { register, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm<ReservationFormData>({
    defaultValues
  });
    ...
    >
      <div className="space-y-2">
        ...
        >
          ...
          <CVRadioOption
            id={Amenity.Soccer}
            value={Amenity.Soccer}
            label="Canchas de Fútbol"
          />
        </CVRadioGroup>
        {errors.amenity && (
          <p className="text-sm text-red-500">{errors.amenity.message}</p>
        )}
      </div>
      {amenity === Amenity.Sum && (
        <>
          <div className="space-y-2">
            ...
            <CVRadioGroup
              className="flex gap-4"
              register={register('shift', { required: 'Debe seleccionar un turno' })}
              value={shift}
            >
              <CVRadioOption
                id={SumShift.Day}
          ...
          <div className="space-y-2">
            ...
            {errors.date_at && (
              <p className="text-sm text-red-500">{errors.date_at.message}</p>
            )}
          </div>
          <div className="space-y-2">
